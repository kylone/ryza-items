...
fn get_item_validation_sets() -> validate_item::ItemValidationSets{
    ...
    let item_validation_sets = validate_item::build_item_validation_sets(&item_list_contents).expect("can't parse item validation lists");

    println!("Parsed lists.yml:");
    println!("  Found {} Item Categories", item_validation_sets.categories.len());
    println!("  Found {} Item Classifications", item_validation_sets.classifications.len());
    println!("  Found {} Elements", item_validation_sets.elements.len());
    println!("  Found {} Gathering Tools", item_validation_sets.gathering_tools.len());
    println!("");
...
fn main() {
    ...
    for file in item_contents {
        ...
        let result = validate_item::validate_item_contents(&file.contents, &item_validation_sets, only_output_invalid);
        if result.is_err() {
