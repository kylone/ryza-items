...
pub fn build_item_validation_sets(contents: &str) ->Result<ItemValidationSets, ScanError> {
    ...
    add_to_set(&doc, "Item Classifications", &mut validation_sets.classifications);
    add_to_set(&doc, "Elements", &mut validation_sets.elements);
    add_to_set(&doc, "Gathering Tools", &mut validation_sets.gathering_tools);
    Ok(validation_sets)
}
...
fn validate_list_values(yaml: &Yaml, validation_set: &HashSet<String>, key: &str, only_output_invalid:bool) -> bool{
    let value = &yaml[key];
    let mut terminal = term::stdout().unwrap();
    ...
    if let Some(list) = value.as_vec() {
        ...
        for value in list{
            if let Some(value) = value.as_str() {
                if validation_set.contains(value) == false{
...
pub fn validate_item_contents(contents: &str, item_validation_sets: &ItemValidationSets, only_output_invalid:bool) -> Result<(), ScanError> {
    let docs = YamlLoader::load_from_str(contents)?;
    // YAML files can actually contain multiple files inside, we want the first one
    let doc = &docs[0];
